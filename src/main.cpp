...
#if defined(NDEBUG)
# error "Memecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0a4f7c899d3178685a264e6ff6d00c26448d5cf695a7cdbf374fc778a9da17bb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Memecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Memecoin Signed Message:\n";
...
{
    // Memecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Memecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Memecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Memecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Memecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Memecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf267b9ce9616dac51f5fd8e6024391bcb3029fa4a59bc7b2ab028b35174b1b97");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Memecoin will revolutionize the meme economy";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04deb52329260b1974e5b239f4d222a686dc4c290d0f3df646696d71965df12411a8f582fe28dde63fcb04246e117b94398fd8143b80ddb134dbf8151f5f3412d2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1534912155;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085102847;

        if (fTestNet)
        {
            block.nTime    = 1534912133;
            block.nNonce   = 385554645;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb29b4cab796fe699e97d040a6a22576f61636e322e0cc96a68a6967515daedb3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xca, 0xbc, 0xd7 }; // Memecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MemecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MemecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MemecoinMiner(CWallet *pwallet)
{
    printf("MemecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("memecoin-miner");
    ...
    try { loop {
        ...
        printf("Running MemecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MemecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MemecoinMiner, pwallet));
}
